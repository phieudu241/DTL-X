	...
	def __init__(self, fin, args,patchfile=None):
		...
		if self.iscompile:
			if os.path.isdir(f"{self.fout}/resources"):
				...
				self.compilecmd = f"java -jar apktool-v3.0.1.jar b -f --aapt assets/aapt2 {self.fout}"
...
regex_for_pairip = [
	...
	[
		r'(\.method public.*onReceive\(Landroid/content/Context;Landroid/content/Intent;\)V\n    (?:\.registers|\.locals)) .[\s\S]*const-string/jumbo.*\s+.*\s+.*\s+(return-void)',
		r'\1 3\n    \2',
        "Remove VMRunner.invoke call"
	],
	[
